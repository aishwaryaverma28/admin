...
const Subscription = ({ onClose }) => {
  ...
  const getData = (startDate, endDate) => {
    axios
      ...
      .then((response) => {
        console.log(response?.data?.data?.subs?.reverse());
  ...
  return (
    ...
    <div className="performance_title2">
      ...
      <span>Total Subscription: {subsCount?.length}</span>
    ...
    <div className="marketing-all-table lead_last_border new_table_1 academy_new_table">
      <table>
        <thead>
          <tr>
            ...
            <th className="common-fonts">REF URL</th>
          </tr>
        </thead>
        <tbody>
          {subsCount?.map((item, index) => (
            <tr key={item?.id}>
              <td className="common-fonts">{item.id}</td>
              <td className="common-fonts">
                {formatDate(item?.creation_date)}
              </td>
              <td className="common-fonts">{item?.ip}</td>
              <td className="common-fonts">{item?.email}</td>
              <td className="common-fonts">
                  <div className="leads_desc">{item?.ref_url}</div>
                </td>
            </tr>
          ))}
        </tbody>
